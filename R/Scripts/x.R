...
#' Performs Mummichog pathway enrichment analysis using both MFN and KEGG databases.
#' Conducts t-tests comparing severe PGD vs no severe PGD groups on untargeted features.
#' Generates pathway enrichment plots and inspection tibbles for visualization.
...
cat(
  ...
  "📊 Combined (MFN + KEGG):\n",
  "   P-value range:    ", min(combined_inspect$p_value), " to ", max(combined_inspect$p_value), "\n",
  "   Enrichment range: ", min(combined_inspect$enrichment), " to ", max(combined_inspect$enrichment), "\n\n",
  strrep("=", 60), "\n\n"
)
