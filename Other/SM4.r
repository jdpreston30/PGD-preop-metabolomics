...
#* Import and preprocess data
  #+ Import Metadata and Sequence File
    metadata <- read_excel("OHT Clinical Data Master Sheet_HC_3_17_25.xlsx",sheet = "Periop") %>%
      select(Patient_no, postop_PGD,Patient) %>%
      rename(PGD = postop_PGD)
    sequence <- read_csv("sequence.csv")
  #+ Create Untargeted Feature Table
    #- Function for processing feature tables
    ...
    #- Import and process untargeted FTs
      HILIC_raw <- read_csv("Raw/HILIC_untargeted_mediansum.csv")
      C18_raw <- read_csv("Raw/C18_untargeted_mediansum.csv")
      HILIC <- process_feature_table(HILIC_raw, sequence, "HILIC")
      C18 <- process_feature_table(C18_raw, sequence, "C18")
    #- Further prepare feature tables for joining with metadata
      HILIC_processed <- HILIC %>%
        mutate(Patient_no = str_extract(Sample_ID, "(?<=H)\\d+")) %>% # Extract number
        mutate(Patient_no = as.numeric(Patient_no)) %>% # Convert to numeric
        select(Patient_no, everything(), -c(Sample_ID, Comment:"L1 ChannelSelect", "Sample Type")) %>%
        arrange(Patient_no) %>%
      ...
      C18_processed <- C18 %>%
        ...
        arrange(Patient_no) %>%
        filter(!is.na(Patient_no))
    #- Join, 1/2 min replace NA features, log2 transform
      UFT_C18_HILIC <- C18_processed %>%
        left_join(HILIC_processed, by = "Patient_no") %>%
        select(where(~ any(. != 0, na.rm = TRUE))) %>%
        mutate(across(where(is.numeric), ~ ifelse(. == 0, 0.5 * min(.[. > 0], na.rm = TRUE), .))) %>%
        left_join(metadata %>% select(Patient_no, PGD), by = "Patient_no") %>%
        mutate(across(-c(Patient_no,PGD), log2)) %>%
        mutate(Patient_no = paste("H", Patient_no, sep = ""))%>%
        select(Patient_no, PGD, everything())
    #- Export
      write.csv(UFT_C18_HILIC, "UFT_C18_HILIC.csv",row.names = FALSE)
  #+ Create targeted feature table (confirmed by standards)
    #- Import Standard library for IDX
      IDX_lib <- read_csv("IDX_lib.csv") %>%
        select(CNAME, KEGGID, HMDBID, Column, Adduct,KEGG_col_adduct, mz_lib,rt_lib)
    #- Transpose UFT back to features being in rows
      UFT_C18_HILIC_transposed <- UFT_C18_HILIC %>%
        select(-PGD) %>% # Remove the second column
        pivot_longer(cols = -Patient_no, names_to = "Feature", values_to = "Value") %>%
        pivot_wider(names_from = Patient_no, values_from = Value) %>%
        separate(Feature, into = c("Column", "mz", "rt"), sep = "_", convert = TRUE)
    #- Write function to match untargeted features to targeted library with threshold arguments
      filter_and_annotate_features <- function(feature_table, lib_table, mz_threshold = 5, rt_threshold = 10) {
        # Step 1: Filter features based on Column, m/z threshold (PPM), and RT threshold
        filtered_features <- feature_table %>%
          inner_join(lib_table, by = "Column",relationship = "many-to-many") %>% # Ensures Column matches first
          filter(
            abs(mz - mz_lib) <= (mz_threshold * mz / 1e6) & # PPM-based mass threshold
              abs(rt - rt_lib) <= rt_threshold # Retention time threshold
          )
        # Step 2: Add metadata columns
        annotated_features <- filtered_features %>%
          select(CNAME, KEGGID, HMDBID, Column,Adduct, KEGG_col_adduct, mz_lib, rt_lib, mz, rt) %>%
          mutate(CNAME_KEGG_HMDB_col_ad = paste(CNAME, KEGGID, HMDBID, Column,Adduct,mz,rt, sep = "_"))
        # Step 3: Rejoin patient intensity data
        final_output <- annotated_features %>%
          left_join(feature_table, by = c("Column", "mz", "rt")) %>%
          mutate(
            mz_time = paste0(mz, "_", (round(rt, 0))))
        return(final_output)
      }
    #- Run function to filter and annotate features
      targeted_FT_transposed <- filter_and_annotate_features(UFT_C18_HILIC_transposed, IDX_lib, mz_threshold = 5, rt_threshold = 30)
      write.csv(targeted_FT_transposed, "targeted_FT_transposed.csv", row.names = FALSE)
    #- Examine features annotated in both modes
    #! 70 detected in both modes, regardless of adduct
    C18_HILIC_duplc <- targeted_FT_transposed %>%
      mutate(across(c(CNAME, KEGGID, HMDBID), ~ na_if(., "NA"))) %>%
      filter(!is.na(Column)) %>%
      group_by(CNAME, KEGGID, HMDBID) %>%
      filter(n_distinct(Column) > 1) %>%
      ungroup() %>%
      arrange(CNAME, KEGGID, HMDBID, Column) %>%
      select(CNAME, KEGGID, HMDBID, Column) %>%
      unique() %>%
      nrow()
    #- Examine features who mz,rt,adduct mapped to multiple possible molecules
    #! 64 total that mapped to various adducts or molecules
    duplicate_feature_count <- targeted_FT_transposed %>%
      select(mz, rt) %>%
      group_by(mz, rt) %>%
      filter(n() > 1) %>%
      ungroup() %>%
